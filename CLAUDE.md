...
- **GVL event tracking** (wall mode): Hooks SUSPENDED/READY/RESUMED thread events. SUSPENDED captures backtrace + normal sample. RESUMED re-captures the backtrace (the Ruby stack is unchanged while off-GVL; re-capturing avoids stale frame_start after a buffer swap) and records off-GVL time and GVL contention time as samples with `vm_state` (converted to `%GVL=blocked` / `%GVL=wait` labels by `merge_vm_state_labels!` in `Rperf.stop`).
- **GC phase tracking**: Hooks GC_ENTER/GC_EXIT events. Records GC mark/sweep samples with `vm_state` (converted to `%GC=mark` / `%GC=sweep` labels by `merge_vm_state_labels!` in `Rperf.stop`), with wall time weight, attributed to the stack that triggered GC.
...
- `stats` fields: `sampling_count`, `sampling_total_ns`, `dropped_samples` are all accessed under GVL. `trigger_count` is `_Atomic size_t` — incremented by the signal handler / nanosleep worker, read and cleared by `snapshot` while running (atomic size_t increments are async-signal-safe when lock-free). `dropped_aggregation` has a benign race in `snapshot` (approximate stat).
...
- **atfork child handler windows (theoretical)**: the `pthread_atfork` child handler calls `rb_internal_thread_remove_event_hook` (takes the VM's hook rwlock — deadlocks if another thread held the read lock at fork) and frees worker-owned tables that the parent's worker may have been mid-mutation on (e.g. between `free(ft->buckets)` and the pointer swap in rehash). All windows are a few instructions wide and have never been observed; a full fix would require quiescing the worker around fork, which is not worth the complexity for a profiler.
